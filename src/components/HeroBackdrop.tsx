 ...
 * the two so the headline stays readable over whatever the photograph contains.
 *
 ...
 *  - No fade-in. A photograph animating from `opacity: 0` is not counted as
...
/**
 * Two stacked washes: text sits left and low, so that corner is darkened
 * hardest while the opposite corner keeps more of the photograph.
 *
 * The floors matter more than the peaks. Sampling an 11x11 grid across the hero
 * against a worst-case near-white photograph, these values hold cream and
 * cream-dim text at 6.07:1 at the *worst* point anywhere in the panel — past the
 * 4.5:1 AA threshold, and not merely where text happens to sit today. An earlier
 * draft measured 2.91:1 in the top-right corner, which was fine right up until a
 * line of text wrapped into it.
 *
 * The cost is that a bright photograph gets heavily darkened. That is the honest
 * trade for cream text over an unknown image. A dark, moody bourbon photograph —
 * the likely case — lets you raise the last stop in each gradient and let much
 * more of the image through. Re-check the contrast if you do.
 */
const SCRIM = [
